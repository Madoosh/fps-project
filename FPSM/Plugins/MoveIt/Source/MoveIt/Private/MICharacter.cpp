...
AMICharacter::AMICharacter(const FObjectInitializer& OI)
	: Super(OI.SetDefaultSubobjectClass<UMICharacterMovementComponent>(ACharacter::CharacterMovementComponentName))
{
	...
	OrientDefaultSettings.OrientAngleMultiplier = -0.15f;
	...
	HitImpactVelocityToPhysicsImpulse.GetRichCurve()->AddKey(0, 2000.f);
	HitImpactVelocityToPhysicsImpulse.GetRichCurve()->AddKey(9000, 2000.f);
	HitImpactVelocityToPhysicsImpulse.GetRichCurve()->AddKey(15000, 3000.f);
	...
	HitByCharacterImpactPhysics.BlendOut.SetBlendTime(0.5f);
	...
	ShotImpactPhysics.ImpulseInterpOutRate = 500.f;
...
FVector SavedRagdollActorLocation = FVector::ZeroVector;
...
void AMICharacter::Tick(float DeltaTime)
{
	...
	if (IsBotControlled() && GetLocalRole() == ROLE_Authority && GetCharacterMovement())
	{
		PreviousInput = Input;
...
void AMICharacter::TickRagdoll()
{
	if (IsRagdoll())
	{
		const FName PelvisBoneName = GetMesh()->GetBoneName(1);
		...
		// Send resulting values to server
		if (IsLocallyControlled())
		{
		...
		else
		{
			...
			const FVector Direction = (RagdollActorLocation - PelvisLocation);
			if (Direction.Size() > 100.f)
			{
				const float Magnitude = (GetWorldTimerManager().IsTimerActive(RagdollCorrectionTimerHandle)) ? 50000.f : 10000.f;
				GetMesh()->AddForce(Direction.GetSafeNormal() * Magnitude, PelvisBoneName, true);
			}
		}
	}
}
...
ECollisionResponse SavedResponseToPawn = ECR_MAX;
ECollisionChannel SavedObjectType = ECC_MAX;
ECollisionEnabled::Type SavedCollisionEnabled = ECollisionEnabled::NoCollision;
void AMICharacter::OnStartRagdoll()
{
	...
	GetWorldTimerManager().SetTimer(RagdollCorrectionTimerHandle, 10.f, false);
...
void AMICharacter::OnCollideWith(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	const bool bIsHit = bHitWallEnabled && !bMontageBlockHit && !bRootMotionBlockHit && Speed >= HitWallImpactVelocityThreshold && !GetWorldTimerManager().IsTimerActive(HitWallSoundTimerHandle);
	...
	if (PhysMat)
	{
		...
		UNiagaraSystem* const NiagaraToPlay = bIsHit ? PhysMat->HitNiagara : PhysMat->ScuffNiagara;
