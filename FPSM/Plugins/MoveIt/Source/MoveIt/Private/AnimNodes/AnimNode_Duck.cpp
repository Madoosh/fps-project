...
void FAnimNode_Duck::UpdateBlendSpace(const FAnimationUpdateContext& Context)
{
	...
	if (Character && Character->GetWorld() && Character->GetWorld()->IsGameWorld())
	{
		...
		float Target = 0.f;
		if (CeilingHit.bBlockingHit/* && CeilingHit.ImpactPoint.Z >= HeadLocation.Z*/)
		{
			// Is there any room for us under it?
			...
			{
				// Ceiling trace fitted the head, but does not give a great Z impact point (off center), get it from line trace
