...
struct MOVEIT_API FBoneLookupTable
{
	...
	FTransform ConvertLocalToComponent(FPoseContext& Output)
	{
		...
		//// Init ParentComponentSpace
		//ParentComponentSpace = Output.Pose.operator[](Cache[0]).Inverse();

		//for (int32 i = 1; i < Cache.Num(); i++)
		//{
		//	const FCompactPoseBoneIndex& BoneIndexParentOfParent = Cache[i];
		//	FTransform::Multiply(&ParentComponentSpace, &ParentComponentSpace, &Output.Pose.operator [](BoneIndexParentOfParent));
		//}
