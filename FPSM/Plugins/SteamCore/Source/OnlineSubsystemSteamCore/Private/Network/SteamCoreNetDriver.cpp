...
/*
#include "Network/SteamCoreNetDriver.h"
...
bool UEOSNetDriver::InitBase(bool bInitAsClient, FNetworkNotify* InNotify, const FURL& URL, bool bReuseAddressAndPort, FString& Error)
{
	...
	{
		UE_LOG(LogEOSSubsystem, Error, TEXT("[UEOSNetDriver::InitBase] User is not logged in, defaulting to IPv4"));
...
{	
	if (!FPlatform::IsLoggedIn())
	{
		UE_LOG(LogEOSSubsystem, Error, TEXT("[UEOSNetDriver::InitConnect] User is not logged in, defaulting to IPv4"));
	...
	{
		UE_LOG(LogEOSSubsystem, Error, TEXT("[UEOSNetDriver::InitConnect] UEOSNetDriver failed to initialize! %s"), *Error);
		return false;
	}
	...
	{
		UE_LOG(LogEOSSubsystem, Error, TEXT("[UEOSNetDriver::InitConnect] Attempted to establish a connection to yourself."));
...
bool UEOSNetDriver::InitListen(FNetworkNotify* InNotify, FURL& ListenURL, bool bReuseAddressAndPort, FString& Error)
{
	if (!FPlatform::IsLoggedIn())
	{
		UE_LOG(LogEOSSubsystem, Error, TEXT("[UEOSNetDriver::InitListen] User is not logged in, defaulting to IPv4"));
	...
	{
		UE_LOG(LogEOSSubsystem, Warning, TEXT("[UEOSNetDriver::InitListen] InitListen failed while setting up base information"));
		return false;
	}
	...
	if (CountBits > 0)
	{
		...
		if (m_P2P)
		{
			...
			if (m_Result == EOSResult::EOS_Success)
			{
				...
				UE_LOG(LogEOSSubsystem, VeryVerbose, TEXT("%i BYTES SENT TO: %s"), m_Data.Num(), *Address->ToString(false));
			}
		}
...
void UEOSNetDriver::OnIncomingConnectionRequest(const EOS_P2P_OnIncomingConnectionRequestInfo* data)
{
	...
	if (m_Result == EOS_EResult::EOS_Success)
	{
		...
		UE_LOG(LogEOSSubsystem, Verbose, TEXT("[OnIncomingConnectionRequest] Accepted new incoming connection from: %s, SocketId: %s"), *m_RemotePeer, UTF8_TO_TCHAR(data->SocketId));
	}
}
	...
	for (int32 i = 0; i < NetDriver->ClientConnections.Num(); i++)
	{
		if (UIpConnection* m_Connection = (UIpConnection*)NetDriver->ClientConnections[i])
		{
			if (m_Connection->RemoteAddr->ToString(false) == m_Remote)
			{
				...
				UE_LOG(LogEOSSubsystem, Warning, TEXT("[OnRemoteConnectionClosed] Closed connection with: %s"), *m_Remote);
			}
		}
	}
}
			...
			if (m_PendingDataSize > 0)
			{
				if (EOS_HP2P m_P2P = EOS_Platform_GetP2PInterface(FPlatform::GetPlatformHandle()))
				{
					...
					if (EOS_P2P_ReceivePacket(EOS_Platform_GetP2PInterface(FPlatform::GetPlatformHandle()), &m_Options, &OutPeerId, &OutSocketId, &OutChannel, m_Data.GetData(), &OutBytesWritten) == EOS_EResult::EOS_Success)
					{
						...
						UE_LOG(LogEOSSubsystem, VeryVerbose, TEXT("[ReceiveSinglePacket] %d/%d bytes received from: %s"), OutBytesWritten, m_PendingDataSize, *m_PeerString);
