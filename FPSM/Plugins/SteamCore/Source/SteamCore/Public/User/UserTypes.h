...
public:
	...
	FGetAuthSessionTicketResponse(const GetAuthSessionTicketResponse_t& data)
		: AuthTicket(data.m_hAuthTicket), Result(static_cast<ESteamResult>(data.m_eResult))
	{}
};
...
public:
	...
	FSteamServerConnectFailure(const SteamServerConnectFailure_t& data)
		: Result(static_cast<ESteamResult>(data.m_eResult)), bStillRetrying(data.m_bStillRetrying)
	{}
};
