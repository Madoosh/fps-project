...
public:
	...
	FSteamInventoryEligiblePromoItemDefIDs(const SteamInventoryEligiblePromoItemDefIDs_t& data)
		: Result(static_cast<ESteamResult>(data.m_result)), SteamID(data.m_steamID), NumEligiblePromoItemDefs(data.m_numEligiblePromoItemDefs), bCachedData(data.m_bCachedData)
	{}
};
...
public:
	...
	FSteamInventoryResultReady(const SteamInventoryResultReady_t& data)
		: Handle(data.m_handle), Result(static_cast<ESteamResult>(data.m_result))
	{}
};
...
public:
	...
	FSteamInventoryRequestPricesResult(const SteamInventoryRequestPricesResult_t& data)
		: Result(static_cast<ESteamResult>(data.m_result)), Currency((strlen(data.m_rgchCurrency)>0) ? data.m_rgchCurrency : "")
	{}
};
...
public:
	...
	FSteamInventoryStartPurchaseResult(const SteamInventoryStartPurchaseResult_t& data)
		: Result(static_cast<ESteamResult>(data.m_result)), OrderID(data.m_ulOrderID), TransID(data.m_ulTransID)
	{}
};
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnSteamInventoryStartPurchaseResult, const FSteamInventoryStartPurchaseResult&, data);
